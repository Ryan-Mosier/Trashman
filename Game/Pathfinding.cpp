...
void calculateCosts(Node *start, Node *goal, vector<vector<Node>> &map) {
    for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize; j++) {
            map[i][j].g_cost = 1000000;
        }
    }
}
...
std::vector<Position> convertPath(std::vector<Node *> in) {
    //todo
...
std::vector<Position> a_star(Position startpos, Position goalpos, TileData grid[xsize][ysize]) {
    ...
    start_node = &map[startpos.x][startpos.y];
    goal_node = &map[goalpos.x][goalpos.y];
    calculateCosts(start_node, goal_node, map);
...
std::vector<Node *> a_star_old(Position startpos, Position goalpos, TileData grid[xsize][ysize]) {
    Node *start_node;
    Node *goal_node;
    vector<vector<Node>> map;
    map = convertMap(startpos, goalpos, grid, start_node, goal_node);
